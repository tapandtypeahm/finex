...
<td>From Date (Entry Date) : </td>
				<td>
				 <input autocomplete="off" type="text"  name="start_date" id="start_date" placeholder="Click to select Date!" class="datepicker2" value="<?php if(isset($_SESSION['cFileReport']['from'])) echo $_SESSION['cFileReport']['from']; ?>" />	
                 </td>
</tr>
...
<td>Up To Date (Entry Date) : </td>
				<td>
				 <input autocomplete="off" type="text"  name="end_date" id="end_date" placeholder="Click to select Date!" class="datepicker2" value="<?php if(isset($_SESSION['cFileReport']['to'])) echo $_SESSION['cFileReport']['to']; ?>"/>	
                 </td>
</tr>
					...
					<select id="customer_city_id" name="city_id" class="city"   onchange="createDropDownAreaReports(this.value)">
                        ...
                        <?php
                            ...
                            foreach($cities as $super)
                             ...
                             <option value="<?php echo $super['city_id'] ?>" <?php if(isset($_SESSION['cFileReport']['city_id'])){ if( $super['city_id'] == $_SESSION['cFileReport']['city_id'] ) { ?> selected="selected" <?php }} ?>><?php echo $super['city_name'] ?></option					>
                             <?php } ?>
...
<td>Area : </td>
				<td>
					<select name="area[]" class="city_area selectpicker" multiple="multiple"  id="city_area1" >
                    	 <option value="-1" >--Please Select--</option>
                          <?php
						  if(isset($_SESSION['cFileReport']['city_id'])){
                            $areas = listAreasFromCityIdWithGroups($_SESSION['cFileReport']['city_id']);
                            foreach($areas as $area)
                              {
                             ?>
                             
                             <option value="<?php echo $area['area_id'] ?>" <?php if(isset($_SESSION['cFileReport']['area_id_array'])){ if(in_array($area['area_id'],$_SESSION['cFileReport']['area_id_array'])) { ?> selected="selected" <?php }} ?>><?php echo $area['area_name'] ?></option					>
                             <?php } 
						  }
							 ?>
                    </select>
...
<tr>
	<td>File Status:</td>
    <td>
    	<input  type="radio" name="file_status" id="open" value="1" <?php if(isset($_SESSION['cFileReport']['file_status'])){ if(  $_SESSION['cFileReport']['file_status']==1 ) { ?> checked="checked" <?php }} ?> /> <label style="display:inline-block; top:3px;position:relative;margin-right:10px;" for="open">Open</label>
		<input  type="radio" name="file_status" id="closed" value="2" <?php if(isset($_SESSION['cFileReport']['file_status'])){ if( $_SESSION['cFileReport']['file_status']==2 ) { ?> checked="checked" <?php }} ?> /> <label style="display:inline-block;top:3px;position:relative;" for="closed">Closed</label>
    	<input  type="radio" name="file_status" id="both"  <?php if(!isset($_SESSION['cFileReport']['file_status']) || ($_SESSION['cFileReport']['file_status']!=1 && $_SESSION['cFileReport']['file_status']!=2)){  ?> checked="checked" <?php } ?> /> <label style="display:inline-block;top:3px;position:relative;" for="both">Both</label>
    </td>
</tr>
...
<hr class="firstTableFinishing" />
	...
	<div class="no_print">
 <?php if(isset($_SESSION['cFileReport']['remainder_array']))
{
	
	$emi_array=$_SESSION['cFileReport']['remainder_array'];
    ...
    <tbody>
		...
		foreach($emi_array as $emi)
		{
         ...
         <tr class="resultRow">
            ...
            <td><?php if($emi['date_added']!="1970-01-01" && $emi['date_added']!='0000-00-00') echo date('d/m/Y',strtotime($emi['date_added'])); else echo "NA"; ?>
            </td>
