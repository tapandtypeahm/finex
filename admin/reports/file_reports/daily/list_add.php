...
<h4 class="headingAlignment no_print">Daily File Reports</h4>
<?php 
...
<td>Area : </td>
				<td>
					<select name="area[]" class="city_area selectpicker" multiple="multiple"  id="city_area1" >
                    	 <option value="-1" >--Please Select--</option>
                          <?php
						  if(isset($_SESSION['dFileReport']['city_id'])){
                            $areas = listAreasFromCityIdWithGroups($_SESSION['dFileReport']['city_id']);
                            foreach($areas as $area)
                              {
                             ?>
                             
                             <option value="<?php echo $area['area_id'] ?>" <?php if(isset($_SESSION['dFileReport']['area_id_array'])){ if(in_array($area['area_id'],$_SESSION['dFileReport']['area_id_array'])) { ?> selected="selected" <?php }} ?>><?php echo $area['area_name'] ?></option					>
                             <?php } 
						  }
							 ?>
                    </select>
