...
<td>From Date (Entry Date) : </td>
				<td>
				 <input autocomplete="off" type="text"  name="start_date" id="start_date" placeholder="Click to select Date!" class="datepicker2" value="<?php if(isset($_SESSION['clFileReport']['from'])) echo $_SESSION['clFileReport']['from']; ?>" />	
                 </td>
</tr>
...
<td>Up To Date (Entry Date) : </td>
				<td>
				 <input autocomplete="off" type="text"  name="end_date" id="end_date" placeholder="Click to select Date!" class="datepicker2" value="<?php if(isset($_SESSION['clFileReport']['to'])) echo $_SESSION['clFileReport']['to']; ?>"/>	
                 </td>
</tr>
					...
					<select id="customer_city_id" name="city_id" class="city"   onchange="createDropDownAreaReports(this.value)">
                        ...
                        <?php
                            ...
                            foreach($cities as $super)
                             ...
                             <option value="<?php echo $super['city_id'] ?>" <?php if(isset($_SESSION['clFileReport']['city_id'])){ if( $super['city_id'] == $_SESSION['clFileReport']['city_id'] ) { ?> selected="selected" <?php }} ?>><?php echo $super['city_name'] ?></option					>
                             <?php } ?>
...
<td>Area : </td>
				<td>
					<select name="area[]" class="city_area selectpicker" multiple="multiple"  id="city_area1" >
                    	 <option value="-1" >--Please Select--</option>
                          <?php
						  if(isset($_SESSION['clFileReport']['city_id'])){
                            $areas = listAreasFromCityIdWithGroups($_SESSION['clFileReport']['city_id']);
                            foreach($areas as $area)
                              {
                             ?>
                             
                             <option value="<?php echo $area['area_id'] ?>" <?php if(isset($_SESSION['clFileReport']['area_id_array'])){ if(in_array($area['area_id'],$_SESSION['clFileReport']['area_id_array'])) { ?> selected="selected" <?php }} ?>><?php echo $area['area_name'] ?></option					>
                             <?php } 
						  }
							 ?>
                    </select>
					...
					<select id="agency_id" name="agency_id">
                        ...
                        <?php
                            ...
                            foreach($agencies as $super)
                             ...
                             <option value="ag<?php echo $super['agency_id'] ?>" <?php if(isset($_SESSION['clFileReport']['agency_id'])){ if( "ag".$super['agency_id'] == $_SESSION['clFileReport']['agency_id'] ) { ?> selected="selected" <?php }} ?>><?php echo $super['agency_name'] ?></option>
                             ...
                             <option value="oc<?php echo $com['our_company_id'] ?>" <?php if(isset($_SESSION['clFileReport']['agency_id'])){ if( "oc".$com['our_company_id'] == $_SESSION['clFileReport']['agency_id'] ) { ?> selected="selected" <?php }} ?> ><?php echo $com['our_company_name'] ?></option>
...
<hr class="firstTableFinishing" />
	...
	<div class="no_print">
 <?php if(isset($_SESSION['clFileReport']['file_array']))
{
	
	$emi_array=$_SESSION['clFileReport']['file_array'];
