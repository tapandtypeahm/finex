...
<h4 class="headingAlignment no_print">Daily Loan Reports</h4>
<?php 
					...
					<select id="customer_city_id" name="city_id" class="city"   onchange="createDropDownAreaReports(this.value)">
                        ...
                        <?php
                            ...
                            foreach($cities as $super)
                             ...
                             <option value="<?php echo $super['city_id'] ?>" <?php if(isset($_SESSION['dLoanReport']['city_id'])){ if( $super['city_id'] == $_SESSION['dLoanReport']['city_id'] ) { ?> selected="selected" <?php }} ?>><?php echo $super['city_name'] ?></option					>
                             <?php } ?>
...
<td>Area : </td>
				<td>
					<select name="area[]" class="city_area selectpicker" multiple="multiple"  id="city_area1" >
                    	 <option value="-1" >--Please Select--</option>
                          <?php
						  if(isset($_SESSION['dLoanReport']['city_id'])){
                            $areas = listAreasFromCityIdWithGroups($_SESSION['dLoanReport']['city_id']);
                            foreach($areas as $area)
                              {
                             ?>
                             
                             <option value="<?php echo $area['area_id'] ?>" <?php if(isset($_SESSION['dLoanReport']['area_id_array'])){ if(in_array($area['area_id'],$_SESSION['dLoanReport']['area_id_array'])) { ?> selected="selected" <?php }} ?>><?php echo $area['area_name'] ?></option					>
                             <?php } 
						  }
							 ?>
                    </select>
					...
					<select id="agency_id" name="agency_id">
                        ...
                        <?php
                            ...
                            foreach($agencies as $super)
                             ...
                             <option value="ag<?php echo $super['agency_id'] ?>" <?php if(isset($_SESSION['dLoanReport']['agency_id'])){ if( "ag".$super['agency_id'] == $_SESSION['dLoanReport']['agency_id'] ) { ?> selected="selected" <?php }} ?>><?php echo $super['agency_name'] ?></option>
                             ...
                             <option value="oc<?php echo $com['our_company_id'] ?>" <?php if(isset($_SESSION['dLoanReport']['agency_id'])){ if( "oc".$com['our_company_id'] == $_SESSION['dLoanReport']['agency_id'] ) { ?> selected="selected" <?php }} ?> ><?php echo $com['our_company_name'] ?></option>
...
<tr>
	<td>File Status:</td>
    <td>
    	<input  type="radio" name="file_status" id="open" value="1" <?php if(isset($_SESSION['dLoanReport']['file_status'])){ if(  $_SESSION['dLoanReport']['file_status']==1 ) { ?> checked="checked" <?php }} ?> /> <label style="display:inline-block; top:3px;position:relative;margin-right:10px;" for="open">Open</label>
		<input  type="radio" name="file_status" id="closed" value="2" <?php if(isset($_SESSION['dLoanReport']['file_status'])){ if( $_SESSION['dLoanReport']['file_status']==2 ) { ?> checked="checked" <?php }} ?> /> <label style="display:inline-block;top:3px;position:relative;" for="closed">Closed</label>
    	<input  type="radio" name="file_status" id="both"  <?php if(!isset($_SESSION['dLoanReport']['file_status']) || ($_SESSION['dLoanReport']['file_status']!=1 && $_SESSION['dLoanReport']['file_status']!=2)){  ?> checked="checked" <?php } ?> /> <label style="display:inline-block;top:3px;position:relative;" for="both">Both</label>
    </td>
</tr>
...
<hr class="firstTableFinishing" />
	...
	<div class="no_print">
 <?php if(isset($_SESSION['dLoanReport']['remainder_array']))
{
	
	$emi_array=$_SESSION['dLoanReport']['remainder_array'];
...
<div class="printBtnDiv no_print"><button class="printBtn btn"><i class="icon-print"></i> Print</button></div>     
    ...
    <thead>
        ...
        <th class="heading">No</th>
            ...
            <th class="heading date">Starting Date</th>
		...
		foreach($emi_array as $emi)
		{
         ...
         <tr class="resultRow">
			...
			echo $prefix.".".preg_replace('/[a-zA-Z]+/', '', $emi['file_no']); ?></span> <?php  echo  $emi['file_no']; ?>
             ...
             <td><?php echo $emi['loan_amount']; ?>
            </td>
             <td width="160px"><?php if($emi['loan_scheme']==1) echo $emi['emi'];  else if($emi['loan_scheme']==2){ if(is_array($emi['scheme'])) { foreach($emi['scheme'] as $scheme){ echo $scheme['emi']." X ".$scheme['duration']."<br>"; } } } ?>
            </td>
