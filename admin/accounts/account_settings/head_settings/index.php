...
if(isset($_GET['action']))
{
	if($_GET['action']=='add')
	{
		if(isset($_SESSION['adminSession']['admin_rights']) && (in_array(2,$admin_rights) || in_array(7,					$admin_rights)))
			{
				$result=addHead($_POST["name"],$_POST["parent_id"]);
				
				if($result!="error")
				{
				$_SESSION['ack']['msg']="Head successfully added!";
	...
	if($_GET['action']=='delete')
	{
		if(isset($_SESSION['adminSession']['admin_rights']) && (in_array(4,$admin_rights) || in_array(7,					$admin_rights)))
			{	
				$result=deleteHead($_GET["lid"]);
				if($result=="success")
				{
				$_SESSION['ack']['msg']="Head deleted Successfuly!";
				$_SESSION['ack']['type']=3; // 3 for delete
				}
				else
				{
					$_SESSION['ack']['msg']="Cannot delete Head! Head already in use!";
	...
	if($_GET['action']=='edit')
	{
		if(isset($_SESSION['adminSession']['admin_rights']) && (in_array(3,$admin_rights) || in_array(7,					$admin_rights)))
			{
				$result=updateHead($_POST["lid"],$_POST["head_name"],$_POST["parent_id"]);
				if($result=="success")
				{
				$_SESSION['ack']['msg']="Head updated Successfuly!";
...
$jsArray=array("jquery.validate.js","jquery.jeditable.js","editable/subheadname.js","validators/accountHead.js");
