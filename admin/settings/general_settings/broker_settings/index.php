...
if(isset($_GET['action']))
{
	if($_GET['action']=='add')
	{
		...
		$result=insertBroker($_POST["name"], $_POST["address"], $_POST["contactNo"]);
		if($result=="success")
				{
				$_SESSION['ack']['msg']="Broker successfully added!";
	...
	if($_GET['action']=='delete')
	{
		if(isset($_SESSION['adminSession']['admin_rights']) && (in_array(4,$admin_rights) || in_array(7,					$admin_rights)))
			{	
				$result=deleteBroker($_GET["lid"]);
				if($result=="success")
				{
				$_SESSION['ack']['msg']="Broker deleted Successfuly!";
				$_SESSION['ack']['type']=3; // 3 for delete
				}
				else
				{
					$_SESSION['ack']['msg']="Cannot delete Broker! Broker already in use!";
		...
		if(isset($_SESSION['adminSession']['admin_rights']) && (in_array(3,$admin_rights) || in_array(7,					$admin_rights)))
			{
				...
				$result=updateBroker($_POST["lid"],$_POST["name"], $_POST["address"],  $_POST["contactNo"]);
				if($result=="success")
				{
				$_SESSION['ack']['msg']="Broker updated Successfuly!";
...
$jsArray=array("jquery.validate.js","validators/broker.js");
