...
if(isset($_GET['action']))
{
	if($_GET['action']=='merge')
	{
		if(isset($_SESSION['adminSession']['admin_rights']) && (in_array(2,$admin_rights) || in_array(7,					$admin_rights)))
			{
				$result=mergeArea($_POST["area_array"],$_POST["location"],$_POST['city_id']);
				
				if($result=="success")
				{
				$_SESSION['ack']['msg']="Merge successful!";
	...
	if($_GET['action']=='delete')
	{
		if(isset($_SESSION['adminSession']['admin_rights']) && (in_array(4,$admin_rights) || in_array(7,					$admin_rights)))
			{	
				$result=deleteArea($_GET["lid"]);
				if($result=="success")
				{
				$_SESSION['ack']['msg']="Area deleted Successfuly!";
				$_SESSION['ack']['type']=3; // 3 for delete
				}
				else
				{
				$_SESSION['ack']['msg']="Cannot delete Area! Area already in use!";
		...
		if(isset($_SESSION['adminSession']['admin_rights']) && (in_array(3,$admin_rights) || in_array(7,$admin_rights)))
			{
				...
				$result=updateArea($_POST["lid"],$_POST["location"],$_POST['city_id']);
				if($result=="success")
				{
				$_SESSION['ack']['msg']="Area updated Successfuly!";
...
$jsArray=array("jquery.validate.js","validators/cities.js","dropDown.js");
