...
if(isset($_GET['action']))
{
	if($_GET['action']=='add')
	{
		if(isset($_SESSION['adminSession']['admin_rights']) && (in_array(2,$admin_rights) || in_array(7,					$admin_rights)))
			{
				$result=insertCity($_POST["location"]);
				
				if($result=="success")
				{
				$_SESSION['ack']['msg']="City successfully added!";
	...
	if($_GET['action']=='delete')
	{
		if(isset($_SESSION['adminSession']['admin_rights']) && (in_array(4,$admin_rights) || in_array(7,					$admin_rights)))
			{	
				$result=deleteCity($_GET["lid"]);
				if($result=="success")
				{
				$_SESSION['ack']['msg']="City deleted Successfuly!";
				$_SESSION['ack']['type']=3; // 3 for delete
				}
				else
				{
				$_SESSION['ack']['msg']="Cannot delete City! City already in use!";
		...
		if(isset($_SESSION['adminSession']['admin_rights']) && (in_array(3,$admin_rights) || in_array(7,$admin_rights)))
			{
				...
				$result=updateCity($_POST["lid"],$_POST["location"]);
				if($result=="success")
				{
				$_SESSION['ack']['msg']="City updated Successfuly!";
