...
if(isset($_GET['action']))
{
	if($_GET['action']=='add')
	{
		if(isset($_SESSION['adminSession']['admin_rights']) && (in_array(2,$admin_rights) || in_array(7,					$admin_rights)))
			{
				$result=insertInsuranceCompany($_POST["name"]);
				
				if($result=="success")
				{
				$_SESSION['ack']['msg']="Insurance Company successfully added!";
	...
	if($_GET['action']=='delete')
	{
		if(isset($_SESSION['adminSession']['admin_rights']) && (in_array(4,$admin_rights) || in_array(7,					$admin_rights)))
			{	
				$result=deleteInsuranceCompany($_GET["lid"]);
				if($result=="success")
				{
				$_SESSION['ack']['msg']="Insurance Company deleted Successfuly!";
	...
	if($_GET['action']=='edit')
	{
		if(isset($_SESSION['adminSession']['admin_rights']) && (in_array(3,$admin_rights) || in_array(7,					$admin_rights)))
			{
				$result=updateInsuranceCompany($_POST["lid"],$_POST["name"]);
				if($result=="success")
				{
				$_SESSION['ack']['msg']="Insurance Company updated Successfuly!";
...
$jsArray=array("jquery.validate.js","validators/insuranceCompanies.js");
